...
(function(w, $){
  ...
  function NotificationItem(options){
    var options = this.options = $.extend(this._default, options);
  ...
  NotificationItem.prototype = {
    ...
    createDom: function(options){
      var $item = this.$item = $(
        '<div class="notification__item enter">'+
          (options.message ? '<div class="notification__message">'+ options.message + '</div>': '') +
        '</div>'
      );
      var $close = this.$close = $('<a class="notification__close">x</a>');
      $item.append($close);
      Notification._container.append($item);
    },
    bindEvent: function(options){
      var self = this, duration = options.duration;
    ...
    close: function(fn){
      ...
      var ele = this.$item.removeClass('enter').addClass('leave')[0];
      if(animation){
        ele.addEventListener('webkitAnimationEnd', cb);
  ...
  w.Notification = {
    ...
    _existFlag: false,
    // 新建通知，返回通知实例
    show: function(options, duration){
      var item;
      if(!this._container){
        this._container = $('<div class="notification"></div>').appendTo('body');
      }
      ...
      item = new NotificationItem(options);
      this._items.push(item);
      return item;
    },
    closeAll: function(){
      var items = this._items,i, len;
...
MTA.Util = {
	...
	tips : {
		...
		show : function(cont, id) {
			$('#' + this.conf.container).html(cont).parents("#message").show();
		...
		hide : function() {
			...
			var _mta_closed_sysmsg = GRI.Util.cookie.get('_mta_closed_sysmsg');
			if(MTA.Page.app){
				var loginName = MTA.loginUser.loginName;
				var appId = MTA.Page.app.appId;
				GRI.Util.cookie.save('_mta_closed_sysmsg', _mta_closed_sysmsg+'/'+loginName+'-'+appId+'-'+$(this).attr('sequence'), 'Sun, 18 Jan 2038 00:00:00 GMT;');
			}else{
				GRI.Util.cookie.save('_mta_closed_sysmsg', _mta_closed_sysmsg+'/'+'--'+$(this).attr('sequence'), 'Sun, 18 Jan 2038 00:00:00 GMT;');
			}
    ...
    /**
     * @description 所有需要大小图切换的页面使用初始化和事件绑定类 (修改为在conditions过滤条中显示)
     * @author johnnyzheng(johnnyzheng@tencent.com) quincykwang
     */
    view_switcher_conditions : {
        cgi : {},
        hdl : null,
        container : 'div_item',
        /**
        ...
        init : function(cgi_conf, hdl) {
            ...
            this.tabInit();
            //点击查看大图,初始化选中状态:默认选中第一个
            MTA.Page.Variable.condition_tab_url = cgi_conf[1];
            MTA.Page.Variable.condition_tab_id = 1;
            //eval(this.hdl + '()');
            $('#' + this.container).find('button').first().addClass("current").siblings().removeClass('current');
        },
        ...
        clickAction : function(id){
            this.tabInit();
            MTA.Page.Variable.condition_tab_url = this.cgi[id];
            eval(this.hdl + '()');
            $('#div_item button[id="condition_tab_'+ id +'"]').parent().addClass('current').siblings().removeClass('current');
        },
        ...
        tabInit : function(){
            var self = this;
            $('#'+ self.container).find('button,a').each(function() {
                var id = $(this).attr('id').match(/condition_tab_(\d{1,2})/i)[1];
                $(this).bind('click', function() {
                    MTA.Page.Variable.condition_tab_url = self.cgi[id];
                    MTA.Page.Variable.condition_tab_id = id;
                    $(this).addClass('current').siblings().removeClass('current');
                    eval(self.hdl + '()');
                });
            });
	...
	createUserGroup : function(id, json, callback, extra){
		...
		var footer_html = '<a href="' + MTA.Config.RootPath + '/custom/ctr_group/add?app_id=' + MTA.Page.app.appId + '">创建用户群<i class="icon_newtab"></i></a>&nbsp;&nbsp;|&nbsp;&nbsp;' +
			'<a href="' + MTA.Config.RootPath + '/custom/ctr_group/list_all?app_id=' + MTA.Page.app.appId + '">管理用户群<i class="icon_newtab"></i></a>';
	...
	items_info : {
        ...
        getFooterInfo: function(theme, footerInfo){
            ...
            ('qc' == theme)? footerInfo :
            (MTA.Page.app.portalId == 11 || MTA.Page.app.portalId == 31)? '腾讯移动分析使用IMEI+网卡MAC地址判断Android用户设备的唯一性。': '腾讯移动分析使用IFA(iOS6以下版本使用OpenUDID)+网卡MAC地址判断iOS用户设备的唯一性。';
        },
        showInLayer: function(items, theme, footerInfo){
            $('#'+this.conf.dialogWrap).hide();
 			if(items){
				...
				if ('wechat' == theme){
                    doc += '<div style="margin-top:6px;color: #000;"><b>数据由专业移动应用分析工具<br/><a href="http://mta.qq.com" target="_blank">腾讯移动分析</a>&nbsp;合作提供</b></div>';
                }
 ...
 */
MTA.Selector = function(target, groupName, json, callback,_opt) {
	this.config = {
		threshold : 6,      // 阈值
	...
	this._json = json;
	this.show = '';
	this.multi = ( (_opt && _opt.multi) || false );
	...
	this.getValue = function() {
		...
		if (self._selectedValue != ''){
			values = values.concat(self._selectedValue);
		}
		return values.join(',');
	};
	/**
	...
	for(var i = 0; i <= this._json.seg0.length - 1; i++) {
		html += '<button class="button '+ (this._json.seg0[i].isDefault ? ' current' : '')+'" value="' + this._json.seg0[i].value + '" title="' + this._json.seg0[i].name + '" name="' + this._groupName + '" id="' + this._groupName + '_' + j + '"><div class="ffbug_wrapper">' + this._json.seg0[i].name + '</button>';
	...
	for(var i = 0; i <= this._json.seg1.length - 1; i++) {
		if(has_otherGroup && j >= self.config.threshold){
			...
			html += '<button class="button ' + (this._json.seg1[i].isDefault ? ' current' : '') + '" value="' + this._json.seg1[i].value + '" title="' + this._json.seg1[i].name + '" name="' + this._groupName + '" id="' + this._groupName + '_' + j + '"><div class="ffbug_wrapper">' + this._json.seg1[i].name + '</div></button>';
	...
	if(j > self.config.threshold && has_otherGroup){// && this._json.seg1.length > self.config.threshold) {
		...
		html += '<div class="condition-append">' + '<div class="btn-group" id="' + divWrapMoreId + '">' + '<button class="button " id="trig_' + this._groupName + '_more" value="__others__more" href="javascript:void(0);">其它 <i class="icon_menu"></i></button>' + dropDownHtml + '</div></div>';
	}
	...
	$(this._target).find('button').click(function() {
		if($(this).attr('value') != '__others__more') {
			...
			else if(self.multi == false || $(this).attr('value') == -1){
				self._selectedValue = '';
	...
	if ($('#' + divWrapMoreId).length > 0){
        if (self.config.selectorType == 1){
            ...
            var data = self._json.seg1.slice(seg1_count);
            self._otherSelector = new Gri.MultiSelect({
                'targetId': targetId,
                'letterSelect': false,
                'multi': self.multi,
                'data': data,
                'group_data': ( self._json.seg2 || ''), // jackiezou 20170116 添加分组功能
                'dataValue': 'value',
                'dataText': 'name',
                'callback': function(selectedValue, selectedText){
                    self._selectedValue = selectedValue;
					// 如果是多选，则不去掉已选中的项，否则去掉已选中项。
					if(self.multi == false){
					...
					if( !selectedText || selectedText.length == 0 ){
						$('#trig_' + self._groupName + '_more').html('其他' + ' <i class="icon_menu"></i>').removeClass('current');;
						// 获取已选择的内容，如果有已选择的内容
						if(self.getValue() == '-1'){
							// 点击全部按钮
							$(self._target).find('button[value="-1"]').click();
						}
					}else{
						$(self._target).find('button[value="-1"]').removeClass('current');
...
MTA.Selector.Util = {
	 ...
	 * jackiezou 2017-02-08  发现这部分功能已经去掉了的，前台页面没有展示选择结果的代码
	 * @param {Object}
	 */
	...
	clearSelected : function(groupName, result, obj, showObj) {
		if(result == '0') {
			$('#' + this.selectedContainer).find("label").each(
				function() {$(this).attr('id').match(/lb_(\w+)_\d{1}/i)[1] == groupName && $(this).remove();}
			);
		} else {
	...
	clickAction : function(obj, callback, json, current, ref) {
		...
		if(!$(obj).hasClass('current')) {
			// 如果是单选，则清理所有的样式，然后选中自己，如果是多选，则选中自己，触发更新数据
			if(ref.multi == false){
			...
			if(result == 0) {
				return ''; // 点击的是全部按钮，所以直接返回。
			}
			$(obj).removeClass('current');
			//如果当期小子项全部都被未选中，则全部渠道要被选中，然后回调一次后台查询
			if($.inArray($(obj).attr('value'), current) != -1) {
				current.splice($.inArray($(obj).attr('value'), current), 1);
				current.length == 0 && function() {
					if( ret = $(obj).attr('id').match(/(\w+)_(\d{1})/i)) {
						$('#' + ret[1] + '_' + MTA.Config.Selector.allSuffix).addClass('current');
						self.isAll = true;
					}
				}();
			}
	...
	/**
	 * @description 全部选项 与 其他选项 的排他操作
	 * 如果选择了全部，则把所有其他选项都情况掉
	 */
...
MTA.Condition.prototype = {
	...
	createQueryRange : function(dateObj, func) {
		...
		this._date = new pickerDateRange(dateObj.inputId, {
            //为了在一个页面中使用多个日期选择器，允许传快速链接的id  (add by quincykwang 2014.12.28)
...
MTA.Applist = function(arr, target, defaultAppId) {
	...
	(arr.length > 0) && function() {
		...
		var doc = '';
		for(var i = 0; i < arr.length; i++) {
			if($.inArray(arr[i].AppName +'##'+ arr[i].GroupId, self.frontAppList) === -1){
				self.frontAppList.push(arr[i].AppName+'##'+$.trim(arr[i].GroupId));
			}
		}
		for(var j= 0 ; j < self.frontAppList.length; j++){
			var name = self.frontAppList[j].split('##')[0];
			var GroupId = self.frontAppList[j].split('##')[1];
			if(GroupId==0){
			    //非分组应用不合并显示
			    for(var i = 0; i < arr.length; i++) {
                    if(GroupId == arr[i].GroupId && name == arr[i].AppName){
                        doc += '<li class="'+ self.frontAppList[j].replace(/#/g, '') +'">'+ name +'<div class="sub_menu"><a class="'+MTA.Config.platformMap[arr[i].Portal].css+'" href="#" id="' + arr[i].AppId + '">' + arr[i].AppName+ '</a>';
			    ...
			    //分组应用合并显示
			    doc += '<li class="'+ self.frontAppList[j].replace(/#/g, '') +'">'+ name +'<div class="sub_menu">';
                for(var i = 0; i < arr.length; i++) {
                    if(GroupId == arr[i].GroupId && name == arr[i].AppName){
...
MTA.LbsApplist = function(arr, target, defaultAppId) {
    // appId 初始值
    this.appId = '';
    this.appName = '';
    this.appLogo = '';
    ...
    (arr.length > 0) && function() {
        ...
        for(var i = 0; i < arr.length; i++) {
            if($.inArray(arr[i].AppName +'##'+ arr[i].RegAccount, self.frontAppList) === -1){
                ...
                doc += '<li id="' + $.trim(arr[i].AppName) + arr[i].RegAccount + '" appid="' + arr[i].AppId + '"><a href="javascript:void(0);">'+ $.trim(arr[i].AppName) +'</a></li>';
            }
            if(defaultAppId == arr[i].AppId){
                self.appId = arr[i].AppId;
                self.appName = arr[i].AppName;
                self.appLogo = arr[i].AppLogo;
            }
        ...
        $('#' + target).find('li').each(function(index) {
            $(this).bind('click', function() {
                self.changeApp($(this).attr('appid'));
            });
        });

        //信息初始化
        self.appLogo && $('#app_logo').length > 0 && $('#app_logo').html('<img src="' + self.appLogo + '">');
        self.appName && $('#app_name').length > 0 && $('#app_name').html(self.appName);
     ...
     * @param {Object} callback 回调函数
     */
    this.changeApp = function(appId, callback) {
        //切换站点，改变全局变量的属性值
        for(var i = 0; i < this.appList.length; i++) {
...
MTA.SNSApplist = function(arr,target,defaultAppId){
    // appId 初始值
        this.appId = '';
        this.GroupId = '';
        this.appName = '';
        ...
        (arr.length > 0) && function() {
            ...
            for(var i = 0; i < arr.length; i++) {
                ...
                if(arr[i].GroupId>0){
                    if(tv>=arr.length){
                        self.frontAppList.push($.trim(arr[i].AppName) + arr[i].AppId+'##'+arr[i].RegAccount);
                        doc += '<li id="' + $.trim(arr[i].AppName) + arr[i].AppId + arr[i].RegAccount + '" appid="' + arr[i].AppId + '"><a href="javascript:void(0);">'+ $.trim(arr[i].AppName) +'</a></li>';
                        break;
                    }
                    if(arr[tv].GroupId == arr[i].GroupId){
                        continue;
                    }
                }
                self.frontAppList.push($.trim(arr[i].AppName) + arr[i].AppId+'##'+arr[i].RegAccount);
...
* 新官网应用选择器
**/
MTA.NewApplist = function(arr,target,defaultAppId,isSns){
    // appId 初始值
        this.appId = '';
        this.GroupId = '';
        this.appName = '';
        ...
        (arr.length > 0) && function() {
            ...
            if(isSns){
                ...
                for(var i = 0; i < arr.length; i++) {
                	var key = arr[i].AppName +'##'+ arr[i].GroupId;
                	if (arr[i].GroupId == 0) {
                        key += '##' + arr[i].AppId
					}
                    if($.inArray(key, self.frontAppList) === -1){
                        self.frontAppList.push(key);
                    }
                }
                for(var j= 0 ; j < self.frontAppList.length; j++){
                    var name = self.frontAppList[j].split('##')[0];
                    ...
                    if(GroupId==0){
                        //非分组应用不合并显示
						var AppId = self.frontAppList[j].split('##')[2];
                        for(var i = 0; i < arr.length; i++) {
                            if(GroupId == arr[i].GroupId && name == arr[i].AppName && AppId == arr[i].AppId){
                            	var portalType = arr[i].Portal.substr(1);
                            	name = '<i class="icon_' + (portalType == 1 ? 'android' : 'ios') + '"> '+name+'</i>';
                                doc += '<li class="'+ self.frontAppList[j].replace(/#/g, '') +'" id="'+ self.frontAppList[j].replace(/#/g, '') +'" appid="' + arr[i].AppId + '"><a href="javascript:void(0);" >'+ name +'</a></li>';
                                (defaultAppId == arr[i].AppId) && function() {
                                    self.appId = arr[i].AppId;
                                    self.appName = arr[i].AppName;
                                    self.appLogo = arr[i].AppLogo;
                        ...
                        for(var i = 0; i < arr.length; i++) {
                            if(GroupId == arr[i].GroupId && name == arr[i].AppName){
                                (defaultAppId == arr[i].AppId) && function() {
                                    ...
                                    self.appLogo = arr[i].AppLogo;
                                    self.portalId = arr[i].Portal;
                                    self.groupId = arr[i].GroupId;
                                    self.portalDesc = MTA.Config.platformMap[arr[i].Portal].desc;
                                    //css = MTA.Config.platformMap[arr[i].Portal].css;
                                }();
                                // var newArray = [];
								// newArray[arr[i].GroupId] = arr[i].AppId;
								if( arr[i].Portal == '31' || arr[i].Portal === '11' || app_id === 0) app_id = arr[i].AppId;
                            }
                        }

                        //分组应用合并显示 newArray[GroupId]
                        doc += '<li class="'+ self.frontAppList[j].replace(/#/g, '') +'" id="'+ self.frontAppList[j].replace(/#/g, '') +'"  appid="'+app_id+'"><a href="javascript:void(0);">'+ name +'</a></li>';
                    }
                }
            }
            $('#' + target).length > 0 && ($('#' + target).html(doc));
        ...
        //调用公共气泡层接口
        MTA.Util.popup('btn_pop_app', 'wrap_pop_app', 'ctn_pop_app', 'on');
        //绑定查询接口
        $('#btn_app_search').length > 0 && function(){
            $('#btn_app_search').click(function(){
                var str = ($('#txt_app_search').length > 0 && $('#txt_app_search').val()) || '';
        ...
        this.changeApp = function(appId, callback) {
            ...
            //改变界面的值
            $('#app_name').html(this.appName);
            //如果有回调函数，调用回调函数
            callback && eval(callback + '()');
        }

        /**
        * @description 切换合并应用的系统
        * @param
        */
        this.changeSys = function(){
            var gid = this.groupId ,appid = this.appId, plat = [],cur='',url;
            if (gid == 0) {
            	// 未分组
				var portalType = appid.substr(1);
                plat[portalType] = '<a class="cur" href="'+url+'"><i class="icon_'+MTA.Config.platformMap[this.portalId].css+'"></i></a>';
			} else {
                for(var i = 0; i < this.appList.length; i++) {
                    if(gid == this.appList[i].GroupId) {
                        cur='';
                        (appid == this.appList[i].AppId) && function(){
                            cur='cur';
                        }()
                        parse_r = GRI.Util.parse_url();
                        parse_r.param['app_id'] = this.appList[i].AppId;
                        url = parse_r.path + GRI.Util.convert_params(parse_r.param);

                        plat[this.appList[i].Portal.substr(1)] = '<a class="'+cur+'" href="'+url+'"><i class="icon_'+MTA.Config.platformMap[this.appList[i].Portal].css+'"></i></a>';

                    }
                }
			}
            $("#side_platform").html(plat.join(''));
            return false;
        }
}
				...
				arr.main[o].m_display && function() {
					...
					for(var p in arr.sub) {
						if(arr.sub[p].p_id == arr.main[o].m_id && arr.sub[p].m_display) {
							var isExternalURL = (arr.sub[p].m_url.match(/^https?:\/\/.*/g) != null);
                            var url = isExternalURL ? arr.sub[p].m_url : '/mta' + arr.sub[p].m_url;
                            appId && !isExternalURL && (url += GRI.Util.convert_params({app_id: appId}, url));
...
MTA.NewMenu = function(arr, target) {
	...
	arr && function() {
		...
		var appId =  MTA.Util.getAppId();
		parse_r = GRI.Util.parse_url();
        parse_r.param['app_id'] = appId;
		for(var o in arr.main) {
				arr.main[o].m_display && function() {
                    html +=  '<li class="item ' + (arr.main[o].m_is_expanded == 1 ? ' show':'') + '">';
                    html +=  '<a href="javascript:" id="parent_menu_'+arr.main[o].m_id+'">';
                    html +=  '<div class="icon-wp"><i class="' + arr.main[o].m_icon_css + '"></i></div>';
                    html +=  '<span class="text">'+ arr.main[o].m_name +'</span><em class="arrow"></em></a>';
                    html += '<ul class="side-sub-list" id="child_menu_'+arr.main[o].m_id+'">';
                        for(var p in arr.sub) {
                            if(arr.sub[p].p_id == arr.main[o].m_id && arr.sub[p].m_display) {
                                var isExternalURL = (arr.sub[p].m_url.match(/^https?:\/\/.*/g) != null);
                                var url = isExternalURL ? arr.sub[p].m_url : '/mta' + arr.sub[p].m_url;
                                appId && (url += GRI.Util.convert_params(parse_r.param, url));
                                html += '<li class="sub-item"><a id="menu_' + arr.sub[p].m_id + '" href="'+url+'">'+arr.sub[p].m_name+(arr.sub[p].m_is_new ? '<i class="attach-icon new"></i>' : (arr.sub[p].m_is_hot ? '<i class="attach-icon hot"></i>' : ''))+'</a></li>';
                            }
                        }
                    html += '</ul></li>';
				}();
	...
	//菜单的收折效果
	$('#' + target).find('a[id^="parent_menu_"]').each(function() {
		$(this).unbind('click').bind('click', function() {
			child = 'child_menu_' + $(this).attr('id').match(/parent_menu_(\d{1})/i)[1];
			var _self_parent = $(this).parent();
			if(!_self_parent.hasClass("show")) {
				_self_parent.addClass('show');
				//处理选中菜单的父级样式
				var cur_tab = self.getMenuInfo(current_action);
				if(cur_tab.p_id == $(this).attr('id').match(/parent_menu_(\d{1})/i)[1]) {
                    cur_tab && function(){
                        $("#parent_menu_"+cur_tab.p_id).parents('.item').addClass('cur');
                    }();
				}
                self.saveMenuStatus();
			} else {
                _self_parent.removeClass('show');
                self.saveMenuStatus();
			}
		});
	});
    ...
    this.saveMenuStatus = function(){
        var closeMenuId = '';
        $('ul[id^=child_menu_]').each(function(){
            if($(this).is(':hidden')) {
                var menuId = $(this).attr('id').replace('child_menu_', '');
                closeMenuId += menuId + ',';
            }
    ...
    this.recoverMenuStatus = function(){
        ...
        if(closeMenuId){
            $('ul[id^=child_menu_]').each(function(){
                var menuId = $(this).attr('id').replace('child_menu_', '');
                if(closeMenuId.indexOf(menuId) > -1){
                    $(this).parent().removeClass('show cur');
                }
                else
                {
        ...
        var closeMenu = GRI.Util.cookie.get('mtaIsCloseMenu');
        !MTA.isIframe && $(".sidebar").mCustomScrollbar();
        if(closeMenu ==1){
            $("#content").addClass("small-sidebar");
            !MTA.isIframe && $(".sidebar").mCustomScrollbar("destroy");
        }
    };
}

MTA.LbsMenu = function(menu, target) {
...
//            };
//        }
        for(var o in this.menuList) {
            if(matchUrl(url, this.menuList[o].m_url)) {
                //获得父级菜单的名称
//                var p_name = function() {
//                    if(self.menuList.sub[o].m_is_child) {
//                        return self.menuList.sub[self.menuList.sub[o].p_id].m_name;
...
//                }();
                //将其与子菜单信息合并成一个对象
                return $.extend(true, this.menuList[o], {
                    'p_name' : '腾讯移动分析'
                });
            }
        }
    ...
    menu && function() {
        ...
        for(var o in menu) {
            ...
            menu[o].m_display && function() {
                var url = '/mta' + menu[o].m_url;
                appId && (url += GRI.Util.convert_params({app_id: appId}, url));
                html += '<div class="mod_sidemenu">'+
                    '<h3 class="title" id="parent_menu_'+menu[o].m_id+'">'+
                    '<a href="' + url + '">'+
                    '<i class="' + menu[o].m_icon_css + '"></i>'+ menu[o].m_name +
                    '</a></h3><div id="child_menu_'+menu[o].m_id+'" style="position:relative;zoom:1"></div></div>';
            }();
...
MTA.Data = {
	...
	loadBasic : function(cgi, hdl, useDate, extra){
		...
		params = $.extend(true, _conditions , params );
	...
	loadChart : function(cgi, chartOpt, container, height, callback) {
		...
		$.getJSON(url, function(data) {
            ...
            //防止嵌套请求遮罩被清除，另外加mask4content遮罩
            if($('#mask4content')[0]) {
                GRI.Util.loading.clear('content');
            }
        ...
        function formatNum(v, ignore){
            ...
            if ( null == v['num'] || (v['num'].indexOf("%") < 0 && isNaN(v['num'])) ){
            	  if (MTA.Config.theme == 'wechat') {
                	...
                	return "<dd class='ui_trendgrid_number' style='font-size:12px;'><strong>无数据或计算中...</strong><em class='ui_trendgrid_unit'></em><em class='ui_trendgrid_change' style='opacity: 0;'>+0</em></dd>";
            	  }
            }
	 ...
	 * @param chartOpts
	 */
	loadKeyIndex_overview : function (data_url, container, chartOpts) {
		if (undefined === chartOpts){
			chartOpts = {'theme':'wechat'};
		}
		var params = [];
		...
		$.getJSON(url, {}, function(result) {
			...
			var con = $("#" + container);
			con.addClass("sep-" + result['data'].length);

			$('#' + container + ' li.item-wp').remove();

			var html = "";
			for (var i = 0; i < result['data'].length; i++) {
				var v = result['data'][i];
				var chart_con = container + "_key_index_chart_" + v['key'];

				html += '<li class="item-wp"><div class="item"><div class="ui_trendgrid_item">';
				html += "<div class='ui_trendgrid_chart' id='"+chart_con+"'></div>";
				html += "<dl>";
				html += "<dt><b title=''>"+v['name']+"</b></dt>";
			...
			if (chartOpts['theme'] != 'wechat'){
				for (var k in result['chart']){
					var chart_con = container + "_key_index_chart_"+k;
					MTA.Data.showKeyIndexChart(result['chart'][k], chartOpts, chart_con);
				}
			}
			if (typeof(MTA.key_index_ad) == 'function'){
	...
	//应用概览使用新样式的实时数据
	loadRealData_overview : function (container) {
		var params = [];
		var data_url = '/overview/ctr_single_app/real?chartType=NewUser';
		params = $.extend(true, params, MTA.Page.Params);
		var url = MTA.Config.RootPath + data_url + GRI.Util.convert_params(params, data_url) + "&rnd="+(+new Date());

		/*格式化整型数值*/
		function formatNumber(num, scope){

			var unit = '';
			scope = scope && scope.toUpperCase();
			if ((scope == 'W' || scope == 'K') && num > 10000000){
				unit = 'W';
				num = Math.round(num / 10000);
			}else if (scope == 'K' && num > 100000){
				unit = 'K';
		...
		/*获取上升或下降图标样式*/
		function getUpDownIcon(data){
			return  data ? data == '-' ? '' : '<i class="icon_up" title="上升"></i>'
				: '<i class="icon_down" title="下降"></i>';
		}

		function flashChangeNum(containerId, num){
			$('#' + containerId + ' .ui_trendgrid_change').animate({opacity: 0}, 750, '', function(){
				$('#' + containerId + ' .ui_trendgrid_change').html(num).animate({opacity: 1}, 750, '', function(){
					$('#' + containerId + ' .ui_trendgrid_change').animate({opacity: 0}, 1500);
				});
			});
		}

		function showChangeNum(containerId, num){
			$('#' + containerId + ' .ui_trendgrid_change').html(num).animate({opacity: 1}, 750, '', function(){
				$('#' + containerId + ' .ui_trendgrid_change').animate({opacity: 0}, 1500);
			});
		}
		...
		/*更新关键指标数值*/
		function updateIndexHtml(oldValue, newValue, containerId){
			...
			if (oldValue != newValue){
				var d = newValue == '-'? (0 - oldValue) : oldValue == '-'? newValue : newValue - oldValue;
				var prefix = d > 0 ? '+' : '-';
				var o = formatNumber(d);
				var val = formatNumber(newValue);
				$('#' + containerId + ' strong').html(val.num);
				$('#' + containerId + ' .ui_trendgrid_unit').html(val.unit);
				flashChangeNum(containerId, prefix + o.num + o.unit);

				return false;
			}else{
				flashChangeNum(containerId, '+0');
			}

			return true;
		}

		function createIndexHtml(value, unit, iconHtml, rate, today){
			var html = '<div class="ui_trendgrid_item">' +
							'<dl>' +
								'<dd class="ui_trendgrid_number">' +
									'<strong>' + value + '</strong>' +
									'<em class="ui_trendgrid_unit">' + unit + '</em>' +
									'<em class="ui_trendgrid_change"></em>' +
								'</dd>' +
								'<dd>日' + iconHtml + rate +  '</dd>' +
							'</dl>' +
							'<div class="ui_trendgrid_time">' + today + '</div>' +
						'</div>';
			return html;
		}

		//心跳
		var timeoutProcess,delay,minTime;
		var delay=minTime=4000,maxTime=300000;
		var cacheData;
		var speed=150;
		function heat(){
			var heatUrl = "/overview/ctr_single_app/heat";
			//避免定时刷新导致频繁记录日志，这里加上no_log标识且设为true
			$.getJSON(MTA.Config.RootPath + heatUrl + GRI.Util.convert_params(params, heatUrl) + "&rnd="+(+new Date()),
				{no_log: true},
				function(data){
					var noDataChange = true;
					if (cacheData){
						noDataChange = updateIndexHtml(cacheData.NewUser, data.NewUser, container+'_NewUser') && noDataChange;
						noDataChange = updateIndexHtml(cacheData.ActiveUser, data.ActiveUser, container+'_ActiveUser') && noDataChange;
						noDataChange = updateIndexHtml(cacheData.ActiveUserCui, data.ActiveUserCui, container+'_QQUser') && noDataChange;
						noDataChange = updateIndexHtml(cacheData.SessionCount, data.SessionCount, container+'_SessionCount') && noDataChange;
					}else{
						noDataChange = false;
					}

					//如果没有数据变化，延迟心跳时间
					noDataChange? (delay < maxTime && (delay += 1000)): (delay = minTime, cacheData = data);
				});

			timeoutProcess = setTimeout(heat, delay);
			//方便外部停止心跳
			MTA.Page.Variable.timeoutProcess = timeoutProcess;
		}

		function start(){

			timeoutProcess && clearTimeout(timeoutProcess);

			$.getJSON(url, {}, function(data){
					var val = formatNumber(data.NewUser);
					$('#' + container + ' .ui_trendgrid_item').remove();
					$('#' + container + '_NewUser').append($(createIndexHtml(val.num, val.unit, getUpDownIcon(data.NewUserUp), data.NewUserPercent, params.end_date)));
					val.num != '-' && showChangeNum('tdNewUser', '+1');
					val = formatNumber(data.ActiveUser);
					$('#' + container + '_ActiveUser').append($(createIndexHtml(val.num, val.unit, getUpDownIcon(data.ActiveUserUp), data.ActiveUserPercent, params.end_date)));
					val.num != '-' && showChangeNum('tdActiveUser', '+1');

					val = formatNumber(data.ActiveUserCui);
					$('#' + container + '_QQUser').append($(createIndexHtml(val.num, val.unit, getUpDownIcon(data.ActiveUserCuiUp), data.ActiveUserCuiPercent, params.end_date)));
					val.num != '-' && showChangeNum('tdQQUser', '+1');

					val = formatNumber(data.SessionCount);
					$('#' + container + '_SessionCount').append($(createIndexHtml(val.num, val.unit, getUpDownIcon(data.SessionCountUp), data.ActiveUserPercent, params.end_date)));
					val.num != '-' && showChangeNum('tdSessionCount', '+1');
	...
	//应用概览使用新样式的图表接口
	loadChart_overview : function(cgi, chartOpt, container, height, callback) {
		var params = [];
		params = $.extend(true, params, MTA.Page.Params);
		var url = MTA.Config.RootPath + cgi + GRI.Util.convert_params(params, cgi)+"&rnd="+(+new Date());
	 ...
	 * @params {String} 容器ID
	 */
	loadPageTable : function(cgi, container, hdl, pre_hdl) {
		...
		GRI.Util.loading.show('main');
		$.getJSON(url, {}, function(result) {
			pre_hdl && pre_hdl(result); // jackiezou 20161130 如果有预处理的逻辑，则先对数据做预处理。
            var spanColIndex = result.spanColIndex == false ? null : '0';
			var dt = Gri.initDataTable({
				tableId : container,
				data : result.data,
...
MTA.DropDownList = function(data, size, callback){
    ...
    var itemsHtml = '';
	if(data.openSearch){
		// 如果是搜索模式，则添加搜索输入框   jackiezou 201703-30
		itemsHtml += '<li><span class="ui_search" style="margin:5px 10px;"><input style="width: 100%"  name="DropDownListKeyword" type="text" placeholder="搜索"><button name="DropDownListSearch" type="submit"></button></span></li>';
	}
    var selectItem = '';
	...
	// jackiezou 2017-03-30 添加搜索框事件
	$('#' + divDropDownListId +' input[name="DropDownListKeyword"]').keyup(function(event){
		var Keyword = $(this).val();
		console.log(Keyword);
		if(Keyword){
			$('#' + divDropDownListId +' li[title]').hide();
			$('#' + divDropDownListId +' a:contains("'+Keyword+'")').parent().show();
		}else{
			$('#' + divDropDownListId +' li').show();
		}
		return false;
	});
...
MTA.Weixin = {
    Util: {
        ...
        */
        recalcHeight :  function(src) {
            MTA.Weixin.Util.source = src || '';
        ...
        expire : function(){
            if ((typeof(Iframe) != "undefined") && Iframe.post){
                var cmd = 'expire';
                if (MTA.Weixin.Util.source == 'wxopen'){
                    cmd = 'showLogin';
                }

                Iframe.post({ type: cmd }, location.hostname);
            }
        },
		setHeight : function(){
			var height;
...
//                if(event.ctrlKey || event.shiftKey || event.altKey) return true;
    ...
    return {
        ...
        validate: function( elemId ){
			// jackiezou 2017-02-15 添加elem参数，当指定elem时，只需要校验一个elem
			if( elemId ){
				for(var i in rules){
					var r = rules[i];
					if(r.elem == elemId){
